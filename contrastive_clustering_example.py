#!/usr/bin/env python3
"""
Example script for clustering YaTC dataset using MAEContrast contrastive representations
"""
...
from rp_clustering import ContrastiveModelExtractor, ClusteringPipeline, VisualizationHelper
...
def create_tsne_visualization(representations, true_labels, output_dir, perplexity=30, max_iter=1000):
    ...
    print("\n4.5. Creating T-SNE visualization of contrastive representations...")
    ...
    plt.title(f'T-SNE Visualization of MAEContrast Contrastive Representations\n'
    ...
    # Save the plot
    tsne_path = os.path.join(output_dir, "tsne_contrastive_representations.png")
...
def run_contrastive_clustering_example():
    """Run clustering on YaTC dataset using MAEContrast contrastive representations"""
    
    # Configuration
    dataset_path = "YaTC_datasets/USTC-TFC2016_MFR"
    # Update these paths to your actual fine-tuned MAEContrast model
    model_path = "contrastive_output_dir/contrastive_model_epoch20.pth"  # Replace with your model path
    ...
    print("="*60)
    print("MAECONTRAST CONTRASTIVE CLUSTERING EXAMPLE")
    print("="*60)
    ...
    # 4. Load MAEContrast model and extract representations
    print("\n4. Loading MAEContrast model and extracting contrastive representations...")
    try:
        extractor = ContrastiveModelExtractor(model_path)
        print("   MAEContrast model loaded successfully!")
    except FileNotFoundError:
        print(f"   Model not found at {model_path}")
        print("   Please update the model_path variable with your actual fine-tuned MAEContrast model path")
        return
    ...
    rep_path = os.path.join(output_dir, "contrastive_representations.npy")
    ...
    for config in clustering_configs:
        ...
        viz_helper.plot_clusters_2d(
            representations, 
            cluster_labels, 
            title=f"{config['name']} - MAEContrast Contrastive Representations",
    ...
    print(f"\nMAEContrast contrastive clustering analysis completed! Results saved to: {output_dir}")
...
def analyze_clustering_vs_true_labels(results, true_labels, output_dir):
    ...
    ax1.set_title('Adjusted Rand Score vs True Labels (MAEContrast)')
    ...
    ax2.set_title('Normalized Mutual Information vs True Labels (MAEContrast)')
    ...
    plt.tight_layout()
    comparison_path = os.path.join(output_dir, "contrastive_clustering_vs_true_labels.png")
...
def save_clustering_results(results, representations, true_labels, output_dir):
    ...
    summary_path = os.path.join(output_dir, "contrastive_clustering_summary.json")
...
if __name__ == "__main__":
    run_contrastive_clustering_example() 