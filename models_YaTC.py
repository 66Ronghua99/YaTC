...
class MAEContrast(nn.Module):
    """ MAE Contrastive Learning Model
    Uses the same encoder structure as MAE but with 3 fully connected layers for contrastive learning
    """

    def __init__(self, img_size=40, patch_size=2, in_chans=1,
                 embed_dim=192, depth=4, num_heads=16,
                 mlp_ratio=4., norm_layer=nn.LayerNorm, 
                 projection_dim=128, hidden_dim=256):
        super().__init__()

        # --------------------------------------------------------------------------
        # MAE encoder specifics (same as original MAE)
        self.patch_embed = PatchEmbed(img_size, patch_size, in_chans, embed_dim)
        self.num_patches = self.patch_embed.num_patches * 5
        ...
        # --------------------------------------------------------------------------
        # Contrastive learning projection head (3 fully connected layers)
    ...
    def forward_encoder(self, x, mask_ratio=0.0):
        ...
        # masking: length -> length * mask_ratio (set to 0 for contrastive learning)
        if mask_ratio > 0:
            x, mask, ids_restore = self.random_masking(x, mask_ratio)
        else:
            mask = None
    ...
    def forward_features(self, x):
        ...
        Returns: [B, embed_dim] - CLS token features
        """
        # Use encoder without masking for contrastive learning
    ...
    def forward(self, x):
        ...
        Returns: [B, projection_dim] - projected features for contrastive loss
        """
        features = self.forward_features(x)  # [B, embed_dim]
        projected = self.projection_head(features)  # [B, projection_dim]
        return projected
...
# for contrastive fine-tuning
def MAEContrast_YaTC(**kwargs):
    model = MAEContrast(
        img_size=40, patch_size=2, in_chans=1, embed_dim=192, depth=4, num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
